...
impl HerdrClient for MockClient {
    ...
    fn workspace_create(&self, cwd: &Path, label: &str) -> anyhow::Result<String> {
        ...
        self.created
            .borrow_mut()
            .push((cwd.to_path_buf(), label.into()));
        self.workspaces.borrow_mut().push(Workspace {
            workspace_id: id.clone(),
            label: label.into(),
        });
        self.panes_by_ws.borrow_mut().insert(
            id.clone(),
            vec![Pane {
...
fn focuses_existing_workspace() {
    ...
    client.workspaces.borrow_mut().push(Workspace {
    ...
    assert_eq!(
        outcome,
        DispatchOutcome::FocusedExisting("ws-existing".into())
    );
...
fn creates_workspace_and_runs_launch_command() {
    ...
    assert_eq!(
        outcome,
        DispatchOutcome::CreatedAndLaunched("ws-foo".into())
    );
    assert_eq!(
        *client.created.borrow(),
        vec![(
            PathBuf::from("/tmp/ghq/github.com/mayaton/foo"),
            "foo".to_string()
        )]
    );
    assert_eq!(
        *client.ran.borrow(),
        vec![("pane-foo".to_string(), "claude".to_string())]
    );
}
...
fn blocks_when_direnv_not_allowed() {
    ...
    let outcome = dispatch(
        ...
        &Config::defaults(),
        &client,
        DirenvStatus::NotAllowed,
    )
    .unwrap();
    assert_eq!(
        outcome,
        DispatchOutcome::BlockedByDirenv(PathBuf::from("/tmp/ghq/github.com/mayaton/foo/.envrc"))
    );
    assert!(client.created.borrow().is_empty());
    assert!(client.ran.borrow().is_empty());
}
...
fn quotes_launch_command_arguments() {
    ...
    assert_eq!(
        *client.ran.borrow(),
        vec![("pane-foo".to_string(), "claude --model 'a b'".to_string())]
    );
}
